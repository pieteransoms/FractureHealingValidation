...
for tissueComposition in tissueCompositions:
    ...
    E  = (EWovBone-EConnTiss)*wovBoneConc**3 + (ESoftCart-EConnTiss)*softCartConc**3 + (EMnrlCart-EConnTiss)*mnrlCartConc**3 + EConnTiss*connTissConc**3                             #based on experimental relation from Carter and Hayes (1977), used by Shefelbine et al. (2005) and Simon et al. (2011)
...
for node in range(len(commonNodesCallusBone)):
    ...
    myModel.Tie(adjust=ON, 
        master=Region(nodes=myAssembly.instances['Bone'].nodes.sequenceFromLabels((masterNodeLabel, )), ), 
        name='CallusNode'+str(slaveNodeLabel), positionTolerance=1.0, positionToleranceMethod=SPECIFIED, 
